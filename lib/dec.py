...
class DEC(nn.Module):
    ...
    def fit(self, X, y=None, lr=0.001, batch_size=256, num_epochs=10, update_interval=1, tol=1e-3):
        ...
        if y is not None:
            y = y.cpu().numpy()
            print("Kmeans acc: %.5f, nmi: %.5f" % (metrics.acc(y, y_pred), normalized_mutual_info_score(y, y_pred)))
        ...
        for epoch in range(num_epochs):
            ...
            if epoch%update_interval == 0:
                ...
                if y is not None:
                    print("acc: %.5f, nmi: %.5f" % (metrics.acc(y, y_pred), normalized_mutual_info_score(y, y_pred)))
                    ACC_all.append(metrics.acc(y,y_pred))
        ...
        print("#Epoch %3d: max_acc: %.4f" % (
            epoch + 1, max(ACC_all)))
        x = torch.linspace(1, len(ACC_all), steps=len(ACC_all))
        x = x.numpy()
        y_acc = torch.FloatTensor(ACC_all).numpy()
        plt.cla()
        plt.plot(x, y_acc, c='red', label='acc')
        plt.savefig('./result/cdec_acc_log.jpg')
